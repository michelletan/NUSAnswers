...
function update_admin_id($id_param, $login_id_param)
{
    if (get_active_role() == USER_ROLE_ADMIN) {
        ...
        $query = "UPDATE admins SET login_id = '" . $login_id . "' WHERE admin_id = '" . $id . "';";
        $db->query($query);
        echo $query;
    }
}
...
function update_admin_account($id_param, $login_id_param, $hashed_password_param)
{
    if (get_active_role() == USER_ROLE_ADMIN) {
        global $db;
        $id              = $db->escape_string($id_param);
        ...
        $query           = "UPDATE admins SET login_id = '" . $login_id . "', hashed_password = '" . $hashed_password . "' WHERE admin_id = '" . $id . "';";
        $db->query($query);
    }
}
